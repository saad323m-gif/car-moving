...
function updateHeaderDateTime() {
    ...
    if (dateEl) dateEl.textContent = now.toLocaleDateString("ar-EG");
...
if (closeSearchBtn && globalSearchBar) {
...
if (globalSearchInput) {
    globalSearchInput.addEventListener("input", async () => {
        ...
        if (keyword.length < 2) return; // تجاهل البحث القصير

        // مثال بحث داخل Firestore (قسم السيارات)
        const carsRef = collection(db, "cars");
        const q = query(carsRef, where("plate", ">=", keyword), where("plate", "<=", keyword + "\uf8ff"));

        const results = await getDocs(q);

        console.log("نتائج البحث:", results.size);

        // لاحقًا نعرض النتائج في نافذة مخصصة
    });
}
...
// تحميل بيانات السيارات من Firestore
async function loadCars() {
    const carsRef = collection(db, "cars");
    const q = query(carsRef, orderBy("plate"));
    ...
    carsList.innerHTML = ""; // تفريغ القائمة قبل إعادة التحميل
...
// تشغيل التحميل عند فتح قسم السيارات
...
async function loadUsers() {
    ...
    usersList.innerHTML = ""; // تفريغ القائمة قبل إعادة التحميل
...
// تشغيل التحميل عند فتح قسم المستخدمين
document.addEventListener("DOMContentLoaded", () => {
    loadUsers();
});
// تحميل بيانات حركة السيارات من Firestore
async function loadMovements() {
    const movesRef = collection(db, "movements");
    const q = query(movesRef, orderBy("outTime", "desc"));

    const snapshot = await getDocs(q);

    const movesList = document.getElementById("movements-list");
    if (!movesList) return;

    movesList.innerHTML = ""; // تفريغ القائمة قبل إعادة التحميل
    ...
    snapshot.forEach(doc => {
        ...
        item.innerHTML = `
            <div class="move-car">${move.carPlate}</div>
            <div class="move-driver">${move.driver}</div>
            <div class="move-destination">${move.destination}</div>
            <div class="move-out">${move.outTime}</div>
            <div class="move-return">${move.returnTime || "—"}</div>
            <div class="move-status">${move.status}</div>
        `;
...
// تشغيل التحميل عند فتح قسم الحركة
document.addEventListener("DOMContentLoaded", () => {
    loadMovements();
});
// إضافة حركة جديدة
const addMoveForm = document.getElementById("add-movement-form");

if (addMoveForm) {
    addMoveForm.addEventListener("submit", async (e) => {
        e.preventDefault();

        const carPlate = document.getElementById("move-car").value;
        const driver = document.getElementById("move-driver").value;
        const destination = document.getElementById("move-destination").value;
        const outTime = document.getElementById("move-out-time").value;

        if (!carPlate || !driver || !destination || !outTime) {
        ...
        addMoveForm.reset();
        loadMovements(); // تحديث القائمة
    });
}
// تسجيل عودة السيارة
async function returnCar(moveId) {
    const returnTime = new Date().toLocaleString("ar-EG");

    const moveRef = doc(db, "movements", moveId);
    ...
    alert("تم تسجيل عودة السيارة بنجاح");
    loadMovements(); // تحديث القائمة
}
// حذف حركة
async function deleteMovement(moveId) {
    const confirmDelete = confirm("هل أنت متأكد من حذف هذه الحركة؟");

    if (!confirmDelete) return;

    await deleteDoc(doc(db, "movements", moveId));
...
async function updateDashboardStats() {
    ...
    const movesSnap = await getDocs(collection(db, "movements"));
    ...
    movesSnap.forEach(doc => {
        const move = doc.data();
        if (move.status === "خارج") activeMoves++;
        if (move.status === "عاد") completedMoves++;
    });
    ...
    if (elActive) elActive.textContent = activeMoves;
    if (elCompleted) elCompleted.textContent = completedMoves;
}

// تشغيل الإحصائيات عند تحميل الصفحة
document.addEventListener("DOMContentLoaded", () => {
...
// نظام شاشة التحميل (Loading Overlay)
...
// التحديث اللحظي لبيانات الحركات
...
// التحديث اللحظي لبيانات السيارات
...
// التحديث اللحظي لبيانات المستخدمين
...
// تشغيل التحديث اللحظي عند تحميل الصفحة
document.addEventListener("DOMContentLoaded", () => {
    listenToMovements();
    listenToCars();
    listenToUsers();
});
// التحكم في الصلاحيات داخل الواجهة
...
// تشغيل الصلاحيات بعد تسجيل الدخول
...
if (globalSearchInput) {
    globalSearchInput.addEventListener("input", async () => {
        ...
        // البحث في السيارات
        const carsRef = collection(db, "cars");
        const qCars = query(carsRef, where("plate", ">=", keyword), where("plate", "<=", keyword + "\uf8ff"));
        const carsSnap = await getDocs(qCars);

        carsSnap.forEach(doc => {
            const car = doc.data();
            results.push({
                title: `🚗 سيارة: ${car.plate}`,
                sub: car.model || ""
            });
        });
        ...
        // البحث في المستخدمين
        const usersRef = collection(db, "users");
        const qUsers = query(usersRef, where("name", ">=", keyword), where("name", "<=", keyword + "\uf8ff"));
        ...
        // البحث في الحركات
        const movesRef = collection(db, "movements");
        const qMoves = query(movesRef, where("destination", ">=", keyword), where("destination", "<=", keyword + "\uf8ff"));
        const movesSnap = await getDocs(qMoves);

        movesSnap.forEach(doc => {
            const move = doc.data();
            results.push({
                title: `📍 حركة: ${move.destination}`,
                sub: `${move.carPlate} — ${move.driver}`
            });
        });
...
// تنظيف البحث عند تغيير التبويبات أو إغلاق الشريط
...
// تنظيف البحث عند تغيير التبويبات
document.querySelectorAll(".tab").forEach(tab => {
    tab.addEventListener("click", () => {
        clearSearch();
    });
});
// تحسين تجربة التبويبات (Tabs UX)
document.querySelectorAll(".tab").forEach(tab => {
    tab.addEventListener("click", () => {
        ...
        // تنظيف البحث عند تغيير التبويب
        clearSearch();
    });
});
// حماية الواجهة من الأخطاء (UI Safety Guards)
function safeGet(id) {
...
// منع الأخطاء عند الضغط على عناصر غير موجودة
document.addEventListener("click", (e) => {
    const target = e.target;

    if (!target) return;

    // منع الأخطاء لو الزر ملوش وظيفة
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // إخفاء شاشة التحميل بعد الإقلاع
    setTimeout(() => hideLoading(), 500);
});
// التعامل مع الأخطاء العامة في النظام
...
window.addEventListener("unhandledrejection", (e) => {
    ...
    showNotification("تعذر تنفيذ العملية", "error");
});
