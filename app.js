...
function normalizePhone(phoneRaw) {
  if (!phoneRaw) return "";
  // إزالة أي شيء غير رقم (يشمل الأرقام العربية والإنجليزية)
...
async function handleSearch(e) {
  ...
  addSection("التحركات", movesRes, (m) => {
    return `
      ...
      <div><strong>السائق:</strong> ${m.userName}</div>
      <div><strong>الحركة:</strong> ${m.action}</div>
  ...
  addSection("العهدة", assignmentsRes, (a) => {
    return `
      ...
      <div><strong>التاريخ:</strong> ${formatTimestampMs(a.createdAt || 0)}</div>
    `;
  });
